...
The quarter-turn-plane certificate near the end of the finite source-vector
section supports the theorem "The regular four-source torsor supplies its
direction plane" in
`Sigma_Same_Section_Transition_Star_Realization_from_Finite_Complex_Equivalence_full_proof.tex`.
It verifies that the cyclic source action already contains a nonzero
two-dimensional orthogonal direction sector; no geometric direction chart is
an additional algebraic input.
-/
...
/-- The selected source vector has the exact equal-weight squared norm used
by the intrinsic direction-plane normalization. -/
theorem transverseSourceVector_inner_self :
    inner ℝ transverseSourceVector transverseSourceVector = 2 := by
  change
    (∑ i : Fin 4,
      (![1, 0, -1, 0] : Fin 4 → ℝ) i *
        (![1, 0, -1, 0] : Fin 4 → ℝ) i) = 2
  norm_num [Fin.sum_univ_four, Matrix.cons_val_zero,
    Matrix.cons_val_one, Matrix.cons_val_two, Matrix.cons_val_three,
    Matrix.cons_val_fin_one]

/--
Machine-checked quarter-turn-plane certificate for the equal-weight
four-source carrier.  The vector and its cyclic image are nonzero and
orthogonal, while the second cyclic image is its negative.  Thus the real
`C4` source action supplies the rank-two direction sector used by the
paper-facing isotypic projector construction.
-/
theorem fourSourceDirectionPlane_certificate :
    inner ℝ transverseSourceVector transverseSourceVector = 2 ∧
      inner ℝ transverseSourceVector
        (fourSourceAugmentationCycle transverseSourceVector) = 0 ∧
      fourSourceAugmentationCycle
          (fourSourceAugmentationCycle transverseSourceVector) =
        -transverseSourceVector := by
  refine
    ⟨transverseSourceVector_inner_self,
      transverseSourceVector_inner_cycle, ?_⟩
  simpa [Function.iterate_succ_apply] using
    fourSourceAugmentationCycle_two_transverse
